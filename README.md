...
- 過去問道場は、設定画面からモバイル版／PC版とWebView／外部ブラウザをそれぞれ選択できます。
- 復習予定が古い計算結果で崩れた場合は、復習画面の「予定を再計算」で学習履歴から再構築できます。
- WebViewでは、過去問道場内の出題開始・問題遷移に必要なCookieとサイト内JavaScriptを許可しています。環境によって動作しない場合は外部ブラウザを利用してください。
